...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset-name", default="BytedTsinghua-SIA/DAPO-Math-17k", help="Hugging Face dataset identifier")
    ...
    parser.add_argument("--save-path", default=None, help="Output file path (default: data/dapo_math_17k_<num_samples>.json)")
...
async def collect_dataset(
    ...
    *,
    ...
    for sample in samples:
        ...
        if missing_count <= 0:
            continue
        prompt = build_prompt(sample.get("prompt")[0].get("content"))
...
def resolve_save_path(save_path: Optional[str], num_samples: int) -> Path:
    ...
    return default_dir / f"dapo_math_17k_{num_samples}.json"
